...
class GradientDescentLearningRule(nn.Module):
    ...
    def __init__(self, device, learning_rate=1e-3):
        ...
        self.register_buffer('learning_rate', torch.tensor(float(learning_rate)))
...
class LSLRGradientDescentLearningRule(nn.Module):
    ...
    def __init__(self, args, total_num_inner_loop_steps, use_learnable_learning_rates, init_learning_rate=1e-3):
        ...
        self.args = args
        self.register_buffer('init_learning_rate', torch.tensor(float(init_learning_rate)))
    ...
    def initialise(self, names_weights_dict):
        ...
        for name, param in names_weights_dict.items():
            # 由于 nn.ParameterDict 不支持带 "." 的 key，通常要把 "." 换成 "-"
            safe_name = name.replace(".", "-")

            # 为该层创建一个对应的学习率参数
            # 形状说明：这里创建的是 (Steps+1,) 的向量，意味着每一步有一个标量学习率，控制该层的所有参数
            self.names_learning_rates_dict[safe_name] = nn.Parameter(
                data=torch.ones(self.total_num_inner_loop_steps + 1) * self.init_learning_rate,
                requires_grad=self.use_learnable_learning_rates
            )

        print(f"LSLR Initialized: Created {len(self.names_learning_rates_dict)} learning rate tensors.")
